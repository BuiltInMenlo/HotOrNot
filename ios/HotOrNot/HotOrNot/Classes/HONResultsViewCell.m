...
- (id)initAsTopCell {
	if ((self = [super init])) {
		...
		bgImgView.image = [UIImage imageNamed:@"leaderTableHeader"];
...
- (id)initAsResultCell:(HONChallengeResultsState)state {
	if ((self = [super init])) {
		...
		bgImgView.image = [[UIImage imageNamed:@"rowGray_nonActive"] stretchableImageWithLeftCapWidth:0.0 topCapHeight:10.0];
		[self addSubview:bgImgView];
		...
		switch (state) {
			case HONChallengesWinning:
				resultImgViewAsset = @"greatJob";
				break;
				
			case HONChallengesLosing:
				resultImgViewAsset = @"tryAgain";
				break;
				
			case HONChallengesTie:
				resultImgViewAsset = @"notBad";
				break;
...
- (id)initAsBottomCell {
	if ((self = [self init])) {
		...
		bgImgView.image = [UIImage imageNamed:@"footerTableRow_nonActive"];
		[self addSubview:bgImgView];
		...
		[dailyChallengeButton setBackgroundImage:[UIImage imageNamed:@"dailyChallenge_nonActive"] forState:UIControlStateNormal];
		[dailyChallengeButton setBackgroundImage:[UIImage imageNamed:@"dailyChallenge_Active"] forState:UIControlStateHighlighted];
