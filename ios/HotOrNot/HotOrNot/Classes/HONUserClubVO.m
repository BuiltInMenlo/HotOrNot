...
//	NSLog(@"NAME:[%@]\nSUBMISSIONS:[%@]:\n=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=\n", [dictionary objectForKey:@"name"], [dictionary objectForKey:@"submissions"]);
	...
	vo.ownerImagePrefix = [[HONAPICaller sharedInstance] normalizePrefixForImageURL:([[dictionary objectForKey:@"owner"] objectForKey:@"avatar"] != nil) ? [[dictionary objectForKey:@"owner"] objectForKey:@"avatar"] : [[HONAPICaller s3BucketForType:HONAmazonS3BucketTypeAvatarsCloudFront] stringByAppendingString:@"/defaultAvatar"]];
	...
	for (NSDictionary *dict in [dictionary objectForKey:@"pending"])
		[pending addObject:([[dict objectForKey:@"extern_name"] length] > 0) ? [HONUserVO userFromContactUserVO:[HONContactUserVO contactWithDictionary:dict]] : [HONUserVO userWithDictionary:dict]];
	...
	for (NSDictionary *dict in [dictionary objectForKey:@"members"])
		[members addObject:[HONUserVO userWithDictionary:dict]];
	...
	for (NSDictionary *dict in [dictionary objectForKey:@"blocked"])
		[banned addObject:[HONUserVO userWithDictionary:dict]];
...
//	}
	...
	vo.clubEnrollmentType = (vo.ownerID == [[[HONUserAssistant sharedInstance] activeUserID] intValue]) ? HONClubEnrollmentTypeOwner : HONClubEnrollmentTypeUndetermined;
...
//			NSLog(@"PENDING:(%d) - [%d - %@]", vo.clubID, userVO.userID, userVO.username);
			if (userVO.userID == [[[HONUserAssistant sharedInstance] activeUserID] intValue]) {
