...
/**
 * GSRxMatch represents a single match. It contains the
 * matched value, range, sub groups, and the original
 * string.
 */

@interface GSRxMatch : NSObject
...
@property (retain) NSArray*  groups;   /* Each object is an GSRxMatchGroup. */
@property (retain) NSString* original; /* The full original string that was matched against.  */
@end


@interface GSRxMatchGroup : NSObject
...
@interface NSRegularExpression (GameShare)
...
/**
 * Replaces all occurances of a regex using a block. The block receives a GSRxMatch object
 * that contains all the details of the match and should return a string
 * which is what the match is replaced with.
 *
 * ie.
 * NSString* result = [RX(@"\\w+") replace:@"hi bud" withDetailsBlock:^(GSRxMatch *match){ return [NSString stringWithFormat:@"%i", match.value.length]; }];
 *  => @"2 3"
 */

- (NSString *)replace:(NSString *)string withDetailsBlock:(NSString *(^)(GSRxMatch *match))replacer;
...
/**
 * Returns an NSArray of GSRxMatch* objects. Each match contains the matched
...
/**
 * Returns the first match as an GSRxMatch* object.
 *
 * ie.
 * NSString* str = @"My email is me@example.com and yours is you@example.com";
 * Rx* rx = RX(@"\\w+[@]\\w+[.](\\w+)");
 * GSRxMatch* match = [rx firstMatchWithDetails:str];
 */

- (GSRxMatch *)firstMatchWithDetails:(NSString *)str;
...
/**
 * Replaces all occurances of a regex using a block. The block receives an GSRxMatch
 * object which contains all of the details for each match and should return a string
 * which is what the match is replaced with.
 *
 * ie.
 * NSString* result = [@"hi bud" replace:RX(@"\\w+") withDetailsBlock:^(GSRxMatch *match){ return [NSString stringWithFormat:@"%i", match.value.length]; }];
 *  => @"2 3"
 */

- (NSString *)replace:(NSRegularExpression *)rx withDetailsBlock:(NSString *(^)(GSRxMatch *match))replacer;
...
/**
 * Returns an the first match as an GSRxMatch* object.
 *
 * ie.
 * NSString* str = @"My email is me@example.com and yours is you@example.com";
 * GSRxMatch* match = [str firstMatchWithDetails:RX(@"\\w+[@]\\w+[.](\\w+)")];
 */

- (GSRxMatch *)firstMatchWithDetails:(NSRegularExpression *)rx;
