...
#import "NSRegularExpression+GameShare.h"

@implementation NSRegularExpression (GameShare)
...
- (NSString *)replace:(NSString *)string withDetailsBlock:(NSString *(^)(GSRxMatch *match))replacer {
	...
	for (int i=(int)matches.count-1; i>=0; i--) {
		...
		GSRxMatch* match = [self resultToMatch:result original:string];
...
- (GSRxMatch *)resultToMatch:(NSTextCheckingResult *)result original:(NSString *)original {
	GSRxMatch* match = [[GSRxMatch alloc] init];
	...
	for (int i=0; i<result.numberOfRanges; i++){
		GSRxMatchGroup* group = [[GSRxMatchGroup alloc] init];
...
@implementation NSString (GameShare)
...
@implementation GSRxMatch
@synthesize value, range, groups, original;
@end

@implementation GSRxMatchGroup
