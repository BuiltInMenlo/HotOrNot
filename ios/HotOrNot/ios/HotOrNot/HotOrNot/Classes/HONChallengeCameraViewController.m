...
#import "HONCameraSubmitViewController.h"
...
@property (readonly, nonatomic, assign) HONCameraSubmitType selfieSubmitType;
	...
	if ([[result objectForKey:@"result"] isEqualToString:@"fail"]) {
		...
		if (_isUploadComplete) {
			...
			[[[UIApplication sharedApplication] delegate].window.rootViewController dismissViewControllerAnimated:YES completion:^(void) {
				...
				if (_selfieSubmitType == HONCameraSubmitTypeCreateChallenge || _selfieSubmitType == HONCameraSubmitTypeReplyChallenge)
...
- (void)previewViewSubmit:(HONCreateChallengePreviewView *)previewView withSubject:(NSString *)subject {
	...
	HONProtoChallengeVO *protoChallengeVO = [HONProtoChallengeVO protoChallengeWithDictionary:@{@"user_id"		: [[HONAppDelegate infoForUser] objectForKey:@"id"],
																								...
																								@"challenge_id"	: [[NSString new] stringFromInt:(_selfieSubmitType == HONCameraSubmitTypeReplyMessage && _messageVO != nil) ? _messageVO.messageID : (_selfieSubmitType == HONCameraSubmitTypeReplyChallenge && _challengeVO != nil) ? _challengeVO.challengeID : 0],
	...
	[self.navigationController pushViewController:[[HONCameraSubmitViewController alloc] initWithProtoChallenge:protoChallengeVO] animated:YES];
